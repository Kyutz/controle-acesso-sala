...
// Definição da sala atual do dispositivo (este ESP32 está na Sala 102)
const String CURRENT_ROOM_PATH = "/salas/sala102";
const String OTHER_ROOM_PATH = "/salas/sala101"; // Sala a ser verificada para presença
...
void handleRFID() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    if (isAuthorized(uid)) { // Verifica se o UID está na lista de autorizados
      int pessoasPresentesSala102 = getPeopleCountFromFirebase(CURRENT_ROOM_PATH);
      ...
      if (presenteNaSalaAtual) {
        // --- Lógica de SAÍDA ---
        // Se a pessoa já está presente na Sala 102, ela está saindo
        if (Firebase.RTDB.deleteNode(&fbdo, CURRENT_ROOM_PATH + "/presentes/" + uid)) {
          pessoasPresentesSala102--; // Decrementa a contagem local
          if (pessoasPresentesSala102 < 0) pessoasPresentesSala102 = 0; // Garante que não fique negativo

          // Atualiza a contagem de pessoas e o status de ocupação no Firebase
          Firebase.RTDB.setInt(&fbdo, CURRENT_ROOM_PATH + "/numPessoas", pessoasPresentesSala102);
          Firebase.RTDB.setBool(&fbdo, CURRENT_ROOM_PATH + "/ocupada", (pessoasPresentesSala102 > 0)); // Ocupada se > 0 pessoas

          displayAccessInfo(uid, "SAIDA", pessoasPresentesSala102); // Exibe mensagem de saída
          Serial.printf("SAIDA autorizada para UID %s. Pessoas na sala 102: %d\n", uid.c_str(), pessoasPresentesSala102);
        } else {
        ...
        // --- Lógica de ENTRADA ---
        // Pessoa está tentando entrar na Sala 102
        // 2. Verificar se a pessoa está presente na Sala 101 (para evitar dupla contagem)
        bool presenteNaSala101 = isPresentInOtherRoom(uid, OTHER_ROOM_PATH);

        if (presenteNaSala101) {
          // Acesso negado se já está na Sala 101
          displayAccessDeniedOtherRoomSequence("Sala 101");
          Serial.printf("Acesso negado para UID %s. Já está na Sala 101.\n", uid.c_str());
        } else {
          // Pessoa pode entrar na Sala 102 (não está na sala atual, nem na outra sala)
          ...
          if (Firebase.RTDB.setString(&fbdo, presentesPath, String(now * 1000))) {
            pessoasPresentesSala102++; // Incrementa a contagem local
            // Atualiza a contagem de pessoas e o status de ocupação no Firebase
            Firebase.RTDB.setInt(&fbdo, CURRENT_ROOM_PATH + "/numPessoas", pessoasPresentesSala102);
            Firebase.RTDB.setBool(&fbdo, CURRENT_ROOM_PATH + "/ocupada", true); // Sala ocupada

            displayAccessInfo(uid, "ENTRADA", pessoasPresentesSala102); // Exibe mensagem de entrada
            Serial.printf("ENTRADA autorizada para UID %s. Pessoas na sala 102: %d\n", uid.c_str(), pessoasPresentesSala102);
          } else {
